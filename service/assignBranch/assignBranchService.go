...
func NewAssignBranchService(db *sqlx.DB, ctx context.Context) *AssignBranchService {
	return &AssignBranchService{db: db, ctx: ctx}
}
...
func (s *AssignBranchService) FindAllByUserId(userId int64) ([]model.AssignBranch, error) {
	...
	for i := range list {
		b, err := branchSvc.FindByBranchId(list[i].BranchID.Int64)
		if err != nil {
			return nil, err
		}
		b.Passcode.String = ""
