...
func (s *NovaDoctorPatientApptService) Save(prn string, apptSessionType string, doctorId int64, rd vesaliusGeo.AppointmentBookingConfirmation, remark string, conn *sqlx.DB) error {
	db := conn
	if db == nil {
		db = s.db
	}
	patientName, err := s.getPatientName(db, prn)
	if err != nil {
		return err
	}
...
func (s *NovaDoctorPatientApptService) UpdateToReschedule(prn string, apptSessionType string, o vesaliusGeo.AppointmentChangeConfirmation, conn *sqlx.DB) error {
	db := conn
	if db == nil {
		db = s.db
	}
