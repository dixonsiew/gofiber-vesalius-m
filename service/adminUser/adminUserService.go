...
var (
	assignBranchSvc *assignBranchService.AssignBranchService = assignBranchService.NewAssignBranchService(database.GetDb(), database.GetCtx())
	branchSvc       *branchService.BranchService             = branchService.NewBranchService(database.GetDb(), database.GetCtx())
)
...
func (s *AdminUserService) FindAllMobileUserAuditLog(offset int, limit int) ([]model.MobileUserAuditLog, error) {
	query := `
        SELECT ` + utils.GetDbCols(model.MobileUserAuditLog{}, "amu.") + ` FROM AUDIT_MOBILE_USER amu
...
func (s *AdminUserService) FindWithAssignBranchByAdminId(adminId int64) (*model.AdminUser, error) {
	...
	ablist, err := assignBranchSvc.FindAllByAdminId(o.AdminID.Int64)
	if err != nil {
		return nil, err
	}
